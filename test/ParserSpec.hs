...
spec = do
    ...
    describe "array" $ do
        it "parses an array" $ do
            testParse array "[0, 1, 2]"
                `shouldParse` Literal
                    dummyPos
                    (Array [Literal dummyPos (Int 0), Literal dummyPos (Int 1), Literal dummyPos (Int 2)])
        ...
        it "parses a matrix" $ do
            testParse matrix "matrix([1, 0], [0, 1])"
                `shouldParse` Literal
                    dummyPos
                    ( Matrix
                        [ Literal dummyPos (Array [Literal dummyPos (Int 1), Literal dummyPos (Int 0)]),
                          Literal dummyPos (Array [Literal dummyPos (Int 0), Literal dummyPos (Int 1)])
                        ]
                    )
    ...
    describe "lambdaApplication" $ do
        it "parses application" $ do
            testParse lambdaApplication "((a:int):a)(0)"
                `shouldParse` LambdaApplication
                    dummyPos
                    ( LambdaFunc
                        dummyPos
                        [(Identifier dummyPos (T.pack "a"), IntT)]
                        (Expr (Identifier dummyPos (T.pack "a")))
                    )
                    (Literal dummyPos (Int 0))
    ...
    describe "ifStmt" $ do
        ...
        it "parses if statement with body" $ do
            testParse ifStmt "if (true) {\nreturn\n}"
                `shouldParse` IfStmt
                    dummyPos
                    (Just (Literal dummyPos (Bool True)))
                    (Block dummyPos [Stmt (Return dummyPos Nothing)])
                    Nothing
        it "parses if statement with else" $ do
            testParse ifStmt "if (true) {\nreturn\n} else {}"
                `shouldParse` IfStmt
                    ...
                    (Just (Literal dummyPos (Bool True)))
                    (Block dummyPos [Stmt (Return dummyPos Nothing)])
                    (Just (Stmt (ElseStmt dummyPos (Block dummyPos []))))
        it "parses if statement with else if" $ do
            testParse ifStmt "if (true) {\nreturn\n} else if () {}"
                `shouldParse` IfStmt
                    ...
                    (Just (Literal dummyPos (Bool True)))
                    (Block dummyPos [Stmt (Return dummyPos Nothing)])
                    (Just (Stmt (ElseStmt dummyPos (Stmt (IfStmt dummyPos Nothing (Block dummyPos []) Nothing)))))
                    ...
                    ( Just
                        ( Stmt
                            ( ElseStmt
                                dummyPos
                                ( Stmt
                                    (IfStmt dummyPos Nothing (Block dummyPos []) (Just (Stmt (ElseStmt dummyPos (Block dummyPos [])))))
                                )
                            )
                        )
                    )
    ...
    describe "functionDeclaration" $ do
        ...
        it "parses function with body" $ do
            testParse functionDeclaration "func f() {\nreturn\n}"
                `shouldParse` FunctionDef
                    dummyPos
                    (Identifier dummyPos (T.pack "f"))
                    []
                    VoidT
                    (Just (Block dummyPos [Stmt (Return dummyPos Nothing)]))
            ...
            describe "precedence" $ do
                it "multiplication binds tighter than addition" $
                    testParse expr "1 + 2 * 3"
                        `shouldParse` Operation
                            dummyPos
                            ( Add
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (Multiply (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "addition binds tighter than bitwise and" $
                    testParse expr "1 & 2 + 3"
                        `shouldParse` Operation
                            ...
                            ( BitwiseAnd
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (Add (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "bitwise and binds tighter than bitwise or" $
                    testParse expr "1 | 2 & 3"
                        `shouldParse` Operation
                            ...
                            ( BitwiseOr
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (BitwiseAnd (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "bitwise or binds tighter than comparison" $
                    testParse expr "1 == 2 | 3"
                        `shouldParse` Operation
                            ...
                            ( Equals
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (BitwiseOr (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "comparison binds tighter than &&" $
                    testParse expr "1 && 2 == 3"
                        `shouldParse` Operation
                            ...
                            ( And
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (Equals (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "&& binds tighter than ||" $
                    testParse expr "1 || 2 && 3"
                        `shouldParse` Operation
                            ...
                            ( Or
                                (Literal dummyPos (Int 1))
                                (Operation dummyPos (And (Literal dummyPos (Int 2)) (Literal dummyPos (Int 3))))
                            )
                ...
                it "subtraction is left-associative" $
                    testParse expr "1 - 2 - 3"
                        `shouldParse` Operation
                            dummyPos
                            ( Subtract
                                (Operation dummyPos (Subtract (Literal dummyPos (Int 1)) (Literal dummyPos (Int 2))))
                                (Literal dummyPos (Int 3))
                            )
