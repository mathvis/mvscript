...
parseStatement = (Decl <$> try parseDeclaration) 
    <|> (Expr <$> (try parseReturn <|> try parseExpr)) 
    <|> (Comment <$> parseComment) 
    <|> try (parseBlock NoType)

parseStatementInBlock :: MVParser Statement
parseStatementInBlock = (Decl <$> try parseDeclaration) <|> (Expr <$> (try parseReturn <|> try parseExpr)) <|> (Comment <$> parseComment) 
...
parseDeclaration = try parseVarInitialization 
...
parseAssign = do
    ...
    op <- try (symbol "//=") $> (\lhs rhs -> Assignment (IntDivAssign lhs rhs))
      <|> try (symbol "+=")  $> (\lhs rhs -> Assignment (AddAssign lhs rhs))
      <|> try (symbol "-=")  $> (\lhs rhs -> Assignment (SubAssign lhs rhs))
      <|> try (symbol "*=")  $> (\lhs rhs -> Assignment (MulAssign lhs rhs))
      <|> try (symbol "/=")  $> (\lhs rhs -> Assignment (DivAssign lhs rhs))
      <|> try (symbol "%=")  $> (\lhs rhs -> Assignment (ModAssign lhs rhs))
      <|> try (symbol "|=")  $> (\lhs rhs -> Assignment (BitwiseOrAssign lhs rhs))
      <|> try (symbol "&=")  $> (\lhs rhs -> Assignment (BitwiseAndAssign lhs rhs))
      <|> try (symbol "^=")  $> (\lhs rhs -> Assignment (BitwiseXorAssign lhs rhs))
      <|> try (symbol "=")   $> (\lhs rhs -> Assignment (Assign lhs rhs))
...
parseFunctionForwardDeclaration = getSourcePos >>= \pos -> lexeme $ symbol "[fwd]" *> parseFunctionSignature >>=
...
parseLambda = LambdaFunc <$> betweenParentheses parseFunctionArguments <*> (symbol ":" *> (Just <$> (parseBlock (FunctionBlock VoidT) <|> parseStatement))) >>= \expr -> modify (removeArgumentsFromTableLambda expr) >> return expr
...
parseBlock blocktype = do
    ...
    stmts <- many (lexeme parseStatementInBlock)
    symbol "}"
...
parseComment = symbol "#" *> many anySingle
